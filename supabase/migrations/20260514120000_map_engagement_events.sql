-- Anonymous visitors (embed) record engagement; map owners read events for analytics.
...
create table if not exists public.map_engagement_events (
  ...
  meta jsonb null,
  constraint map_engagement_event_type check (
    event_type in (
      'session_start',
  ...
  constraint map_engagement_surface check (surface in ('embed', 'client_preview', 'admin_preview'))
);
...
-- Embed / anon: append-only for published maps; listing must belong to map when set.
...
-- Logged-in map team or admin: read events for reporting.
...
-- Authenticated users cannot insert via client (embed uses anon); keeps writes predictable.
-- Service role bypasses RLS for migrations / ops.
