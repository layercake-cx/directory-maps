...
**Branch/PR:** `feat/2026-09-21-branding-colour-labels` (PR not opened yet)
...
None yet. `generate_directory_site` source no longer emits `--gold` / `--teal`; that takes effect on the next function deploy, then on the next directory republish.

### Rollback plan
Revert the branch/commit. Redeploy the previous `generate_directory_site` if that function was already shipped. Existing `theme_json` rows that still contain `goldColor`/`tealColor` are ignored until a save strips them.

### Verified on staging
- [ ] Branding tab advanced colours show the new labels and no Gold/Teal fields.
- [ ] Changing "Cards and panels" still restyles listing cards after save + republish (visual check).

### Issues / notes
Already-published HTML is unchanged until `generate_directory_site` is deployed and a directory is republished. Published pages never used `--gold`/`--teal`, so dropping those variables is a no-op visually.
...
- [x] **Deployed to production** (`gxixwdjfmegxcxfeflro`) via `supabase functions deploy generate_media_alt_text`, on the user's explicit go-ahead ("deploy to live with these, non invasive changes"). No migration involved.
...
- `supabase/functions/_shared/staticSiteRenderer.ts` (`buildRobotsTxt()`): now emits a named `User-agent` block for GPTBot, ClaudeBot, PerplexityBot and Google-Extended alongside the existing `User-agent: *` block, both mirroring the same directory-wide index/noindex policy (the Settings tab's single "let search engines index this directory" switch) — not a new toggle, just making the existing policy explicit to AI crawlers instead of leaving them to infer it from the wildcard rule.
- `supabase/functions/generate_directory_site/builders.ts`: fixed the "Related entries" row logo, which rendered `alt=""` instead of the `"{name} logo"` fallback already used for every other entry logo on the page.
- Docs: `docs/USER_GUIDE.md` (Directory settings § SEO settings) and `docs/FEATURES.md` (new §4.4a-8) updated to describe the AI-crawler rules.

### Verified
- [x] `deno check` clean on both changed files.
- [x] Local `deno eval` of `buildRobotsTxt()` confirms correct output shape for both the indexable and noindex cases (four named blocks + wildcard, `Sitemap:` line only when indexable).
- [x] Deployed to the test project (`beqejxneehilplrtpntn`) via `supabase functions deploy generate_directory_site --project-ref beqejxneehilplrtpntn` from the shared working directory (confirmed clean `git status` and correct branch immediately before and after — no parallel session interference this time).
...
## 2026-09-18 — [Production] Directory AI intent-driven search

**Branch/PR:** `feat/2026-09-18-directory-ai-search` ([PR #191](https://github.com/layercake-cx/directory-maps/pull/191), merged).
...
- [ ] **The "configured" path (a real prompt + a real Claude call) still not exercised anywhere** — this needs `directories.ai_search_prompt` actually set on a real directory, which means either a live admin/client-portal session (no test credentials available in this session) or a direct DB write; deliberately not done via the service-role key from an agent session without being asked. **This is the single biggest gap before treating this feature as trustworthy in production** — everything verified so far is the not-configured/error/CORS surface, never the actual Claude-calling logic. Once a prompt is set on a real directory, re-test: a real query returns a plausible, real entry-id subset; republishing that directory picks up the debounced client-side call and filters the list correctly; and the keyword fallback still works if the call is made to fail.
...
### Production rollout (2026-09-18, same day, user's explicit go-ahead)
- [x] PR #191 merged to `main` (`gh pr merge --merge`) after resolving the merge conflicts against PR #190 (see below) — GitHub Pages redeployed automatically (`gh run list` confirms success).
- [x] Migration applied to **production** (`gxixwdjfmegxcxfeflro`) — CLI relinked and confirmed (`supabase/.temp/project-ref`), `--dry-run` showed only this one migration pending (matching staging exactly), applied for real, embedded `NOTICE: VERIFY PASSED` printed during the apply. Same integrity-checklist caveat as staging above — not independently re-run.
- [x] `directory_ai_search` and `generate_directory_site` both deployed to **production**. Black-box re-verified against the live production endpoint with the production anon key: nonexistent `directory_id` → `{"entry_ids":null,"disabled":true}` (200), missing `directory_id` → clean 400, `OPTIONS` preflight → 204. Same surface as staging, same result.
- [x] Vercel production deploy (`npm run deploy:live`) — ran without the permission block noted in the 2026-09-18 embed-sidebar incident entry below (that block was hit on a different attempt/context; this one completed directly). Confirmed `maps.layercake-cx.biz` responding 200 with a fresh `x-vercel-id`, and confirmed the new AI-search code (`directory_ai_search_prompt_updated`, the web-search toggle copy) is present in the live production JS bundle.
- [ ] **Still no live functional test of the actual AI search** — see the unchecked item above; this is true in production exactly as it was in staging, now with real visitor traffic able to reach the (currently always-disabled, since no directory has a prompt set) endpoint.

### Merge conflict against PR #190 (resolved before merging)
`main` had moved forward with [PR #190](https://github.com/layercake-cx/directory-maps/pull/190) (horizontal filter bar / two-thirds-width results, entry below) by the time this branch was ready, touching the same regions of `builders.ts`. Resolved via a real `git merge origin/main` (not a rebase) — `builders.ts` and `docs/FEATURES.md` merged automatically with both features' changes intact (verified by grepping for markers from both sides post-merge); `docs/DEPLOYMENTS.md` and `docs/USER_GUIDE.md` needed manual resolution to keep both entries/both layout descriptions. Caught an unintended side effect in the process: an earlier `generate_directory_site` deploy to **staging** (before this merge) shipped from a version of `builders.ts` that predated PR #190's changes, silently reverting the horizontal-filter-bar layout on staging. Fixed by redeploying `generate_directory_site` to staging again immediately after the merge, confirmed via `grep` for `dir-content-row` in a fresh local preview render — restores PR #190's layout alongside this feature's AI-search wiring. Production was never touched by the stale intermediate deploy, so this was staging-only.
...
- [x] Vercel production deploy completed by the user (`npm run deploy:live` from the corrected worktree) — confirmed via `vercel ls directory-maps --prod` showing a fresh `● Ready` production deployment on the real `layercake-apps/directory-maps` project, and `maps.layercake-cx.biz` responding `200`.
...
A second, smaller mix-up: the first `npm run deploy:live` (Vercel) attempt was also run from that same fresh worktree, which had no `.vercel/project.json` — Vercel auto-created and deployed to a **brand-new project** (`directory-maps-embed-sidebar-wt` in the `layercake-apps` team) instead of the real linked `directory-maps` project, and that deploy then failed outright (`fetch failed`). Fixed for the retry by copying the real `.vercel/project.json` into the worktree; the retry itself was correctly blocked by this session's own permission gate for production deploys, so the user ran `npm run deploy:live` themselves from the corrected worktree — confirmed successful. The stray `directory-maps-embed-sidebar-wt` Vercel project was not deleted automatically and is still sitting in the `layercake-apps` team; needs manual cleanup in the Vercel dashboard if unwanted.
