...
| `ai_content_prompt` | `text null` | Admin-authored prompt for AI content generation (§4.8). Null/blank = feature off for this directory. |
| `ai_content_generation_status`, `ai_content_generation_started_at`, `ai_content_generated_at`, `ai_content_generation_error`, `ai_content_generation_total`, `ai_content_generation_processed` | various, all nullable | Persistent status for the directory-wide "Generate all entry content" bulk run (§4.8) — same shape as the `site_generation_*` columns used for publish-site status. |
...
- `directories.theme_json` (§4.1) covers colour tokens/logo/favicon/hero banner — reuses the `maps.theme_json` + CSS-custom-property-injection pattern (§3.7), no new mechanism. Favicon is stored as `theme_json.faviconUrl` and baked into published HTML as `<link rel="icon">` by `generate_directory_site`. Colours, fonts, the logo image and height, and the hero banner are written to `theme.css` (linked from every page) so a visual change overwrites that one file. Hero banner is `theme_json.heroBannerUrl` plus optional `heroBannerHeight` (px); when set, every published page shows a full-width background image behind the header that fades into the page background colour. The published banner is 100px taller than the stored height, and header background colours are rendered at 40% transparency (alpha 0.6) so the image shows through. The site-title wording stays in the HTML.
...
The directory-entry successor to the removed map-level AI search enrichment feature (see `docs/FEATURES.md` §4.4d/§4.4g for the full narrative) — an admin/client-authored prompt (`directories.ai_content_prompt`, §4.1) plus Claude-written page content for entries, reachable automatically (empty entries only), on demand (one entry, always), or as a directory-wide bulk regenerate (every entry, type-to-confirm).

- `entry_content_jobs`: `id uuid`, `entry_id → directory_entries.id`, `directory_id → directories.id`, `status text check in ('pending','processing','completed','failed')`, `requested_by text check in ('auto','bulk')`, `attempt_count int`, `error text null`, `created_at`, `started_at`, `completed_at`. Mirrors the removed `listing_enrichment_jobs` exactly, with `requested_by` replacing `trigger_source`. A single-entry manual "Generate with AI" click bypasses this queue entirely (synchronous Edge Function call) — only the automatic (empty-on-insert) and bulk-regenerate paths ever create a row here.
- `directory_entry_versions`: `id uuid`, `entry_id → directory_entries.id`, `notes_html text null`, `source text check in ('manual','ai_manual','ai_auto','ai_bulk')`, `actor_user_id uuid null → auth.users.id`, `created_at`. Append-only history of every `notes_html` save, written by application code immediately after each successful write (not a DB trigger) — general-purpose, not scoped to AI writes only, so a mistaken bulk regenerate is always recoverable via "Restore" in the entry editor's Content tab.
- RLS: `entry_content_jobs` is admin-only (mirrors `listing_enrichment_jobs` — read/written by service-role Edge Functions, never queried directly from the client). `directory_entry_versions` follows the same admin + own-client pattern as `directory_entries` itself, since the client portal's version-history UI reads it directly.
...
**SEO & AI discoverability.** A published directory must be crawlable by a client that does not execute JavaScript and by an LLM crawler reading `llms.txt`. Given the audit in §3.5 (no SSR/SSG capability exists today), and per the **decision made 2026-07-14**, this is delivered as:
  ...
  Required elements: per-page `<title>`/`<meta description>`, canonical `<link rel="canonical">`, Open Graph + Twitter Card tags, JSON-LD (`LocalBusiness`/`Organization` per entry, `ItemList` for the directory index, `WebPage` + `BreadcrumbList` for content pages), `sitemap.xml`, `robots.txt`, `llms.txt` (directory-level + per-entry summaries), clean crawlable URLs (`/directories/:clientSlug/:directorySlug` for the landing page, `/directories/:clientSlug/:directorySlug/:entrySlug` for entries, `/directories/:clientSlug/:directorySlug/:pageSlug` for top-level content pages, and `/directories/:clientSlug/:directorySlug/:parentSlug/:childSlug` for nested content pages — on a custom domain the `/directories/:client/:dir` prefix is stripped), index/no-index controls at both directory and entry level (§4). Content-page slug and parent changes are recorded in `directory_redirects` (same table as entry slug renames; `old_slug` holds the previous path relative to the directory root) and emitted as 301s via the published `redirects.json` manifest.
...
**Implementation note (2026-08-29, revised): built around explicit per-map/per-directory attachment, not the applies_to column this story originally assumed.** A categorisation defines nothing about where it's usable on its own — `categorisation_attachments` (`20260829040000_create_categorisation_attachments.sql`) records that a specific categorisation is attached to a specific map or directory, and only then can its terms tag records there or drive a filter. The same categorisation can be attached to any number of maps and directories independently, managed via `CategorisationAttachmentPicker.jsx` from either a map's Filters panel or a directory's settings page.

Two earlier passes at this got the model wrong and were corrected the same day (see `docs/DEPLOYMENTS.md` for both): the first let any categorisation flagged `applies_to_listings` tag every map a client owned automatically, with no per-map control; the second wrongly concluded a self-authored map (no directory link) shouldn't get categories at all. Both were reverted. The actual, current model:

- `generate_directory_site`'s previously-inert `exploreFilterBar()` chips are real and toggleable (client-side filtering, no page reload), built only from categorisations currently attached to that directory — detaching one stops it appearing even if entries still carry old tags for it.
- When a directory has a map attached as its live pin datasource (DIR-E4), the same filter action also drives that map via a `postMessage` bridge from the directory's landing page to the map's `<iframe>` — one filter action narrows both surfaces (they share the same `entry_category_terms` tags on the same entries). The map's own filter-bar UI is suppressed in this context (`?hideFilterBar=1`).
- A **self-authored** map (no directory) can independently have categorisations attached to it too, tagging its own `listings` via `listing_category_terms` (peer of `entry_category_terms`) — via the same `CategorisationAttachmentPicker` from its Filters panel, and a `category_<key>` CSV import column mirroring the directory-entry convention.

Files: `src/lib/categorisations.js` (attachment + adapter functions), `src/components/directories/CategorisationAttachmentPicker.jsx`, `src/pages/EmbedMap.jsx`, `src/components/PublishedMapView.jsx` (`externalActiveFilters`/`hideFilterBar` props), `src/pages/client/ClientMapData.jsx`/`src/pages/admin/AdminMapData.jsx` (CSV import), `supabase/functions/generate_directory_site/index.ts`. Migrations: `20260829010000_categorisations_anon_select.sql`, `20260829040000_create_categorisation_attachments.sql`.
...
*Status (2026-09-22):* Keyword search on the published homepage is local full-listing matching (name, slug, body, keywords, website, location, tags). Location search is a separate Settings-tab opt-in (`directories.location_search_enabled`, default off): after republish, the homepage gains a Distance from filter, and the search box can fill it from a place. Natural-language discovery is a separate **Help me choose** control that reuses `directory_ai_search` (`supabase/functions/directory_ai_search/index.ts`) as a conversation: Claude may ask a follow-up or return listing ids (re-validated server-side). Per-directory opt-in (`directories.ai_search_prompt`, blank = off) and optional web search (`directories.ai_search_web_enabled`) are unchanged. Selected ids become the same result set as keyword search and filters (list, count, and map pins via `visibleEntryIds`). Configuration lives on the directory's **AI** tab. See `docs/DATA_AND_PRIVACY.md` §10 for the data flow.
...
| 3 | Natural-language search resolution for DIR-E7 | **LLM-backed parser** (new Edge Function) — requires a `docs/DATA_AND_PRIVACY.md` update before shipping, per AGENTS.md. **Superseded 2026-09-18**: shipped as full-corpus forced-tool-call entry selection (`directory_ai_search`) rather than a structured-predicate parser, plus an optional per-directory web-search opt-in — see DIR-E7-S1's status note. | §5, DIR-E7-S1 |
...
1. **Documentation routing correction (resolved).** `AGENTS.md`, `docs/README.md`, `docs/FEATURES.md`, `docs/DEPLOY.md`, and others previously described `HashRouter`/hash routes, which no longer match the code (`BrowserRouter`, confirmed by `src/Root.jsx` and `src/lib/hashSearchParams.js`). These have since been corrected in a doc-hygiene pass alongside `docs/FRONTEND_ARCHITECTURE.md`.
