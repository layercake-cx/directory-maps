...
- `directories.theme_json` (§4.1) covers colour tokens/logo/favicon/hero banner — reuses the `maps.theme_json` + CSS-custom-property-injection pattern (§3.7), no new mechanism. Favicon is stored as `theme_json.faviconUrl` and baked into published HTML as `<link rel="icon">` by `generate_directory_site`. Hero banner is `theme_json.heroBannerUrl` plus optional `heroBannerHeight` (px); when set, every published page gets a full-width background image behind the header that fades into the page background colour. The published banner is 100px taller than the stored height, and header background colours are rendered at 40% transparency (alpha 0.6) so the image shows through.
