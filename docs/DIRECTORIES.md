...
*Status (2026-09-18):* Shipped as `directory_ai_search` (`supabase/functions/directory_ai_search/index.ts`), with the actual design refined from the predicate-parser sketch above once it came time to build it: rather than parsing the query into a structured filter predicate, the function sends Claude the visitor's query plus every visible entry's data (name, location, coordinates, categorisation terms, a short description) and asks it, via forced tool-calling, to select the matching entry ids directly — every id it returns is re-validated server-side against the real corpus before being handed back, the same safety property the removed map-level "Ask AI" search had. This is a per-directory opt-in (`directories.ai_search_prompt`, admin-authored instructions, blank = off) with an additional opt-in (`directories.ai_search_web_enabled`) letting Claude use Anthropic's web-search tool for outside context — never to introduce an entry that isn't already in the directory. The published site's search box calls this directly (debounced) and falls back to the pre-existing plain keyword match on any failure, timeout, or when a directory hasn't opted in — the "falls back to keyword match" behaviour in the gherkin above is satisfied by reliability fallback rather than by a "query doesn't map to a facet" case, since this design doesn't parse to facets in the first place. Configuration lives on the directory's **AI** tab, alongside AI content generation. See `docs/DATA_AND_PRIVACY.md` §10 for the data flow this adds.
