...
- **Not built:** Publish UI (so none of this is exercises table today), robots.txt/llms.txt/Open Graph/category-or-location index pages (parity with the map feature's own current gaps, not the full brief vision), directory custom domains.
...
| Directory publish pipeline (DIR-E2 generator + routing) | **In production, unreachable without a Publish UI** | `generate_directory_site` Edge Function + `/directories/...` middleware routing deployed to production 2026-08-27 (verified live against `maps.layercake-cx.biz`); no Publish UI exists yet to call `publish_directory`, so nothing here is exercised end-to-end by a real directory today |
