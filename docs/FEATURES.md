...
| Directory branding (DIR-E3) | **Implemented, not yet deployed** | `directories.theme_json` (added Phase 4a) now has a real editor — `DirectoryBrandingPanel.jsx` (client + admin) sets primary colour, header background/text colour, and an optional logo URL. `generate_directory_site` injects these as CSS custom properties (`--brand-primary` etc.) into every generated page and renders a themed header bar on the directory landing page; colour values are validated against a strict hex pattern before being interpolated into the generated `<style>` block (a bad/malicious value falls back to the default rather than breaking the page). Font, corner radius, and favicon tokens from the original brief aren't built yet. Directory custom domains (Phase 4b) already serve this styling, once published. |
...
| Bring Your Own Domain (Epic 4) | **Deployed to staging + production (flagged), Phases 0–2 of 6; Phase 4a (directory generalization) deployed to staging only** | Domain registration, DNS verification, Vercel domain attachment, and host-based routing (`middleware.js` + `resolve_custom_domain()` RPC), behind the `custom_domain` feature flag (off by default) + `maps.custom_domain` entitlement. Verified end-to-end against a real domain on staging; verified via real Vercel edge routing on production (hitting an attached-but-unregistered-in-prod domain correctly returned the app's own fallback, not a Vercel error). No production client has used this yet — the flag is off for everyone by default. SEO metadata generalization, favicon, and Google Analytics are Phases 3–5, not started. Separately: `client_domains`/`resolve_custom_domain()`/`middleware.js` generalized to be entity-polymorphic (map or directory) — deployed to staging 2026-08-27 (migration applied, verified against real staging data; Vercel preview deployed, regression-checked). `DomainSettings.jsx`/`manage_client_domain` (Phase 4b) also generalized — a client can now pick a map or a directory in the "Publishes" dropdown; directory domains gate on the `directories` feature flag (no separate entitlement), map domains keep the existing `maps.custom_domain` entitlement gate unchanged. Implemented, `deno check`/build clean; not yet deployed. |
