...
Lets an organisation represented in a directory claim, verify, and manage its own listing without becoming a directory administrator — a claim grants rights to one directory item, never the directory. 10-phase build (Monday: "Claimed Directory Listings (Epic)"); Phase 0–2 covered here, remaining phases (manual claim lifecycle, magic-link auth, Listing Manager UI, item-only publish isolation, self-service flow, ownership transfer) not started. Payment is deliberately deferred to a separate follow-up epic ("Claim Payments (Stripe)") — the self-service flow, once built, activates a claim on email/domain verification alone, no payment step, until that epic lands.
...
- **Admin/client Claims tab** (Phase 2): Overview / Claims / Settings sub-tabs on a directory's page (owners/managers only), gated by the `claims` entitlement via `EntitlementGate`. Settings tab: enable/disable, price, currency, payment type, intro HTML (reuses the existing `RichTextEditor`/`sanitizeNotesHtml`). Claims list is a UI shell — empty until Phase 3 creates any claims. Admin event `directory_claim_settings_updated` on save.
- No public "Claim this listing" button, no claim lifecycle, no payment, no magic-link auth yet — all later phases.
- No interactive click-through this session (a dev server from another session already owned port 5173; verified via production build only) — recommend the user verify the Claims tab directly.

Files: `src/components/directories/DirectoryClaimsPanel.jsx`, `src/lib/directoryClaimSettings.js`, `src/lib/entitlementMessages.js` (`claims` key). Migrations: `20260923120000_seed_claims_feature_flag.sql`, `20260923121000_gate_claims_entitlement.sql`, `20260923130000_create_claims_schema.sql`.
...
| Claimed Directory Listings (§4.4m) | **Deployed to production (flagged), Phases 0–2 of a 10-phase build** | Lets an organisation claim, verify, and manage one directory item without becoming a directory administrator. Behind the `claims` feature flag (off by default) + `maps.claims` entitlement (Professional+). Phase 0–1 landed the entitlement, event taxonomy, and full domain schema (`claims`, `claim_users`, `claim_payments`, `directory_claim_settings`, `directory_entry_team_members`). Phase 2 (this deploy) adds the admin/client Claims tab (Overview/Claims/Settings) — Settings is fully functional (enable, price, currency, payment type, intro HTML); Claims list is an empty shell until Phase 3. No claim lifecycle, no public CTA, no magic-link auth, no payment yet. Payment is deliberately deferred to a separate follow-up epic. No interactive click-through this session (port 5173 already owned by another session; verified via production build) — recommend the user verify the Claims tab directly. |
