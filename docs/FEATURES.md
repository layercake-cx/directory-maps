...
| Directory publish pipeline (DIR-E2 generator + routing) | **Deployed to staging, unreachable without a Publish UI** | `generate_directory_site` Edge Function + `/directories/...` middleware routing shipped to staging; no Publish UI exists yet to call `publish_directory`, so nothing here is exercised end-to-end by a real directory today |
