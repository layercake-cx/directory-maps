...
- No public "Claim this listing" button, no online payment, no editing of listing content/team members yet — all later phases.
- No interactive click-through of the authenticated flow this session (no test login credentials) — `/claim/login` and the unauthenticated redirect from `/claim/manage/:id` were smoke-tested live; recommend the user verify the full Listing Manager (as a signed-in claim user) directly.

Files: `src/components/directories/DirectoryClaimsPanel.jsx`, `src/lib/directoryClaimSettings.js`, `src/lib/claims.js`, `src/lib/claimManager.js`, `src/pages/claim/ClaimLogin.jsx`, `src/pages/claim/ClaimManager.jsx`, `src/lib/entitlementMessages.js` (`claims` key). Migrations: `20260923120000_seed_claims_feature_flag.sql`, `20260923121000_gate_claims_entitlement.sql`, `20260923130000_create_claims_schema.sql`, `20260923140000_claims_lifecycle_rpcs.sql`, `20260923150000_claim_manager_shell_rpcs.sql`.
...
| Claimed Directory Listings (§4.4m) | **Deployed to production (flagged), Phases 0–4 of a 10-phase build** | Lets an organisation claim, verify, and manage one directory item without becoming a directory administrator. Behind the `claims` feature flag (off by default) + `maps.claims` entitlement (Professional+). Phase 0–1 landed the entitlement, event taxonomy, and full domain schema. Phase 2 added the admin/client Claims tab. Phase 3 made the Claims sub-tab real (admin create/verify/activate/suspend/revoke, manual payment status, `/claim/login` magic-link auth). Phase 4 (this deploy) adds the claim-user-facing **Listing Manager** (`/claim/manage/:claimId`) — a picker when someone's linked to more than one claim, then Listing/SEO/Contact details/Team tabs (read-only previews, item-scoped via `get_claimed_listing()`/`get_claim_team_members()`) and a **Users** tab that's genuinely functional (owner invites/removes editors via `claim_invite_user`/`claim_remove_user`, each an immediate real magic-link send). Preview/Publish are visible but disabled. Every RPC re-checks the caller is linked to that specific claim (`can_access_claim()`/`is_claim_owner()`) since `security definer` bypasses RLS. No public CTA, no online payment, no editing of listing content/team members yet. `/claim/login` and the unauthenticated-redirect guard were smoke-tested live on a Vercel preview; the full authenticated Listing Manager flow has not been click-tested (no test login credentials) — recommend the user verify it directly. |
