...
- **Search query**: a visitor-facing "Ask AI" box on the published/embed map (`search_listings_by_intent` Edge Function), separate from the existing plain-text substring search box. Sends the map's listing corpus (core fields + `listing_research`) plus the visitor's free-text query to Claude Haiku 4.5, forced via tool-call to return only listing ids — the function then validates every returned id against the map's real listing set before responding, so a hallucinated id can never surface a listing that doesn't exist. Matching listings narrow both the map markers and the panel list, and the map auto-fits bounds to just the results. Visibility (`ai_search_enabled`) flows through the existing publish-snapshot pipeline (`buildPublicationConfig`/`normalizePublicationConfig`) — like other map settings, it only takes effect once published, not immediately on save. Only a boolean flows into the public config; the raw enrichment prompt text stays admin-only.
...
Not built yet: the manual "re-run enrichment" admin action, per-listing job-status visibility in the admin UI, and external-LLM-facing structured data (schema.org/JSON-LD or a public feed).
...
| Intent-Based AI Search (Epic 2) | **In development (flagged)** | Enrichment pipeline (job queue, cron worker, Claude Haiku 4.5) + intent search-query feature (Ask AI box, hallucination-guarded id validation, map auto-fit) + `maps.ai_search` entitlement (Professional plan+/Founding Partner, per-client override, server-side enforced) shipped to a feature branch; hidden behind the `ai_search` feature flag. No admin re-run action, no job-status UI yet |
