...
Lets an organisation represented in a directory claim, verify, and manage its own listing without becoming a directory administrator — a claim grants rights to one directory item, never the directory. 10-phase build (Monday: "Claimed Directory Listings (Epic)"); Phase 0–3 covered here, remaining phases (Listing Manager UI, editable content, item-only publish isolation, self-service flow, ownership transfer) not started. Payment is deliberately deferred to a separate follow-up epic ("Claim Payments (Stripe)") — claims reach `active` today only via an admin manually recording payment status; the self-service flow (a later phase) activates on email/domain verification alone, no payment step, until that epic lands.
...
| Claimed Directory Listings (§4.4m) | **Deployed to staging (flagged), Phases 0–6 of a 10-phase build; production pending sign-off on Phase 6** | Lets an organisation claim, verify, and manage one directory item without becoming a directory administrator. Behind the `claims` feature flag (off by default) + `maps.claims` entitlement (Professional+). Phase 0–1 landed the entitlement, event taxonomy, and full domain schema. Phase 2 added the admin/client Claims tab. Phase 3 made the Claims sub-tab real (admin create/verify/activate/suspend/revoke, manual payment status, `/claim/login` magic-link auth). Phase 4 built the claim-user **Listing Manager** shell. Phase 5 made Listing/SEO/Contact details/Team genuinely editable, plus an admin/client Team editor for parity. Phase 6 (this deploy, the epic's highest-risk phase) adds a genuinely isolated **Publish** for a claimed listing — a new `generate_directory_site` scope (`claim_item`) that touches only that one entry's page, never the homepage/sitemap/other entries, enforced server-side by a new `requireDirectoryItemPublishAccess()` check (admin/contact OR the entry's active claim owner/editor). Found and fixed a real pre-existing bug along the way: the site-generation manifest was being overwritten wholesale on every publish regardless of scope, which could silently mark another entry's unpublished edit as "already published" — now merges instead. Also found, and deliberately left unfixed pending its own follow-up, a broader pre-existing gap: `generate_directory_site`'s other scopes have no caller-authorization check at all. No live acceptance test was possible this session (no way to obtain a real user session or service-role key here) — verified via `deno check` and a live anonymous-caller rejection test against staging only; the claim-user and admin publish paths themselves are unverified against real data. No public CTA, no online payment yet. |
