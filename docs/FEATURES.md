...
| Claimed Directory Listings (§4.4m) | **Deployed to production (flagged), Phases 0–5 of a 10-phase build** | Lets an organisation claim, verify, and manage one directory item without becoming a directory administrator. Behind the `claims` feature flag (off by default) + `maps.claims` entitlement (Professional+). Phase 0–1 landed the entitlement, event taxonomy, and full domain schema. Phase 2 added the admin/client Claims tab. Phase 3 made the Claims sub-tab real (admin create/verify/activate/suspend/revoke, manual payment status, `/claim/login` magic-link auth). Phase 4 built the claim-user **Listing Manager** shell (`/claim/manage/:claimId`, a picker for multiple claims, item-scoped reads via `get_claimed_listing()`/`get_claim_team_members()`, a functional Users tab). Phase 5 (this deploy) makes Listing/SEO/Contact details/Team **genuinely editable** — three new RPCs share a `require_active_claim_editor()` precondition (linked AND the claim is `active`) and stamp provenance on every write so a future AI enrichment job knows not to overwrite claimant-managed content; team-member CRUD is owner-or-editor (unlike the owner-only Users tab). Client/admin parity: the same team-members table also got a `TeamMembersEditor.jsx` in the ordinary entry-edit Content tab (RLS already covered it, no new RPC). Preview/Publish still visible but disabled. No public CTA, no online payment, no isolated publish yet. `/claim/login` and the unauthenticated-redirect guard were smoke-tested live; the full authenticated edit flow has not been click-tested (no test login credentials) — recommend the user verify it directly. |
