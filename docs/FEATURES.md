...
Not built yet: generalized SEO metadata/canonical-URL resolution per custom domain (Phase 3), favicon config (Phase 4), Google Analytics injection (Phase 5). Deployed to staging only — production `middleware.js` still runs the pre-Epic-4 path-only version pending explicit sign-off, per house policy for this file (it serves 100% of the branded domain's live traffic).
...
| Bring Your Own Domain (Epic 4) | **Deployed to staging (flagged), Phases 0–2 of 6** | Domain registration, DNS verification, Vercel domain attachment, and host-based routing (`middleware.js` + `resolve_custom_domain()` RPC), behind the `custom_domain` feature flag + `maps.custom_domain` entitlement — verified end-to-end against a real domain on staging. Deployed to staging only; production `middleware.js` unchanged pending explicit sign-off. SEO metadata generalization, favicon, and Google Analytics are Phases 3–5, not started |
