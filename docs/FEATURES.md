...
- **Not built (at the time):** Publish UI (so none of this is exercised end-to-end yet — see #129, a separate PR), robots.txt/llms.txt/Open Graph, redirect serving, category-or-location index pages, directory custom domains.
...
| Directory publish pipeline (DIR-E2 generator + routing) | **In production, redirects/OG/llms.txt on staging (#129 unmerged)** | `generate_directory_site` + `/directories/...` middleware routing in production since 2026-08-27; redirects, Open Graph tags, `llms.txt` on staging pending merge; verified live against a real published directory (`l-cakez/uk-association-directory`) via a Vercel preview deploy |
