...
| Directory publish pipeline (DIR-E2 generator + routing) | **Core in production; redirects/OG/llms.txt merged to main, not yet deployed to production** | `generate_directory_site` + `/directories/...` middleware routing deployed to production 2026-08-27; redirects, Open Graph tags, `llms.txt` merged to `main` same day but staging-only until a production deploy — verified live against a real published directory (`l-cakez/uk-association-directory`) via a Vercel preview deploy |
| Directory publish UI | **Merged to main, not yet deployed to production** | Publish panel + history/restore on the directory dashboard; pure frontend. Confirmed working end-to-end by the user against a real published directory |
