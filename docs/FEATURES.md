...
- **Not built (at the time):** Publish UI, robots.txt/llms.txt/Open Graph/category-or-location index pages (parity with the map feature's own current gaps, not the full brief vision), directory custom domains.
...
| Directory publish pipeline (DIR-E2 generator + routing) | **In production** | `generate_directory_site` Edge Function + `/directories/...` middleware routing deployed to production 2026-08-27 (verified live against `maps.layercake-cx.biz`) |
| Directory publish UI | **Built, not yet deployed** | Publish panel + history/restore on the directory dashboard; pure frontend, no new migration/function needed |
