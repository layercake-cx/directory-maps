...
### 4.4h Directory AI intent-driven search (new, 2026-09-18)

The directory-entry successor to the removed map-level "Ask AI" search (§4.4d), and the resolution of the DIR-E7-S1 natural-language search requirement (`docs/DIRECTORIES.md`). A visitor's free-text query on a published directory's search box is resolved by Claude against that directory's full entry corpus and categorisation metadata, filtering the visible list down to the matching entries — replacing the plain keyword substring match that search box used until now (that keyword matching remains the automatic fallback whenever AI search isn't configured or its call fails). No feature flag/entitlement gating, same posture as AI content generation — a per-directory opt-in prompt is the only gate, plus a request-count rate limit on the public endpoint.

- **Directory-level search prompt**: a free-text field (`directories.ai_search_prompt`) on the directory's **AI** tab (admin + client portal, second panel alongside AI content generation), describing how the AI should interpret and prioritise queries. Blank = feature off; the published search box then stays on plain keyword matching exactly as before this feature existed.
- **Optional web search opt-in**: a separate switch (`directories.ai_search_web_enabled`, default off) lets Claude use Anthropic's server-side web-search tool for outside context (e.g. an accreditation or place name it doesn't recognise). Hard-constrained, in both the system prompt and a server-side id-validation pass, to never surface an entry that isn't genuinely in the directory — web results can only inform reasoning.
- **Public Edge Function**: `directory_ai_search` — anonymous, CORS-enabled (the published site is a fully static build with no other live backend to extend), called directly by the generated page's inline script. Loads the directory's visible entries (active, not noindexed) and their attached categorisations, sends them plus the query to Claude (`claude-haiku-4-5`) via forced tool-calling (`select_matching_entries`), and — the one safety property carried over unchanged from the removed map feature — filters every returned id against the real entry-id set server-side before responding, so a hallucinated id can never surface a nonexistent entry.
- **Published-site integration**: the existing search box's `input` handler is debounced (~400ms) and, when AI search is configured, calls `directory_ai_search` and restricts/orders the already-rendered rows by the returned ids instead of the prior keyword-scoring pass; facet filtering, the result count, the map sync, and URL state (`?q=`) are all unchanged and continue to combine with whichever search path produced the match. Any failure (network error, timeout, non-2xx, or a disabled response) falls straight through to the same keyword-matching code used when the feature is off — the search box can never appear broken to a visitor.
- **Rate limiting**: `directory_ai_search_requests` logs one row per real Claude call (not per keystroke); the function checks the per-directory count for the last minute before spending an API call, and degrades to the same "not configured" response rather than erroring when over the threshold.
...
| Directory AI intent-driven search (§4.4h, DIR-E7-S1) | **New, 2026-09-18** | Per-directory admin search prompt (`directories.ai_search_prompt`) plus an optional web-search opt-in; visitor queries on the published site are resolved by Claude against the directory's full entry corpus and filter the list down to the matches, with an automatic fallback to plain keyword matching on any failure or when unconfigured. Public `directory_ai_search` Edge Function, rate-limited per directory. |
