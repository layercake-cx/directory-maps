...
- No public "Claim this listing" button, no online payment, no editable listing content yet — all later phases.
- No interactive click-through this session (no test login credentials available) — recommend the user verify the Claims tab and `/claim/login` directly.

Files: `src/components/directories/DirectoryClaimsPanel.jsx`, `src/lib/directoryClaimSettings.js`, `src/lib/claims.js`, `src/pages/claim/ClaimLogin.jsx`, `src/lib/entitlementMessages.js` (`claims` key). Migrations: `20260923120000_seed_claims_feature_flag.sql`, `20260923121000_gate_claims_entitlement.sql`, `20260923130000_create_claims_schema.sql`, `20260923140000_claims_lifecycle_rpcs.sql`.
...
| Claimed Directory Listings (§4.4m) | **Deployed to production (flagged), Phases 0–3 of a 10-phase build** | Lets an organisation claim, verify, and manage one directory item without becoming a directory administrator. Behind the `claims` feature flag (off by default) + `maps.claims` entitlement (Professional+). Phase 0–1 landed the entitlement, event taxonomy, and full domain schema. Phase 2 added the admin/client Claims tab. Phase 3 (this deploy) makes the Claims sub-tab real: admin **Create claim** (select listing, enter owner, verify — domain-email auto-check or admin override — all in one `create_manual_claim` RPC), per-claim Activate/Suspend/Reactivate/Revoke, manual payment-status recording (no Stripe), and a genuinely new claim-user magic-link auth surface (`/claim/login`, `link_claim_user_by_email()`, `get_my_claim_context()`) proving a claim user can sign in and see their linked claims — the restricted Listing Manager UI itself is Phase 4. Every mutating RPC explicitly checks caller permission server-side (`can_manage_client()`), since `security definer` bypasses RLS. No public CTA, no online payment, no editable listing content yet. No interactive click-through this session (no test login credentials) — recommend the user verify the Claims tab and `/claim/login` directly. |
