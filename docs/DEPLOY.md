...
- **Root URL:** The app uses clean-path routing via `BrowserRouter` (`/`, `/login`, etc.). Old hash URLs (`/#/…`) are redirected to their clean-path equivalent automatically by a shim in `index.html`.
