...
The next panel on the **AI** tab shows how many active entries are missing any of their SEO/social fields (meta title, meta description, keywords, social title, social description, AI summary) and lets you fill them all in one go:

1. It tells you upfront how many entries are missing something, before you click anything.
2. **Backfill missing metadata** queues a background job for exactly those entries — never entries that already have everything, so there's nothing to confirm or type first (unlike "Generate all entry content" above, this can't overwrite anything you've written). A progress count shows while it runs.
3. New entries missing any of these fields get them filled in automatically as they're created, without needing this button at all.
4. Each field this fills in shows a small "auto-drafted by AI" note on that entry's own Search & Metadata tab, so you know it's worth a quick look — though there's no obligation to change anything.
...
3. Select a page to edit its **Title**, **URL slug**, **Parent page**, **Navigation label** (optional shorter wording for the header; leave blank to use the title), **Show in site navigation**, **Published**, and its **Content**, written in the same rich text editor entries use. Published pages use that same sizing: paragraphs and lists match, and headings have space underneath them.
...
*(Requires the Directories beta — see above.)* On **Data**, the **Directories** tab lets a map read its pins live from one of your directories instead of its own listings — pick a directory and click **Use this directory**, or click **Build a directory from this map** to copy this map's own groups and listings into a brand-new directory first (you'll review and publish it, then come back here to attach it). **Building a directory from a map also carries across the map's own filter fields as categories** attached to the new directory (single-choice, multiple-choice, and Yes/No fields — free-text fields have no category equivalent and aren't carried across), so entries stay just as filterable as the source map's listings were. The Directories tab is always open, even if a Google Sheets sync is currently active — attaching a directory automatically disconnects that sync, since a map is either self-authored or directory-sourced, never both. There's no separate sync step once attached: the map's published embed shows the directory's published entries as they currently stand, and updates automatically the next time the directory is published — not on every draft edit. **Manual entry**, **Upload CSV**, and **Sync data** lock while a directory is linked; click **Disconnect** on the Directories tab to revert the map to its own listings. Note: a directory-sourced map already shows the directory's own categories as filter chips in its search bar — a directory entry's filterable metadata is **categories** (`categorisations`), the directory-side equivalent of a map's own filter fields, not the same underlying data.
...
| Set the directory header logo height | Directory → Branding → Logo height |
