 ...
 *    'active', map OR directory — client_domains.directory_id/DomainSettings.jsx
 *    since 20260827130000) — resolves which client/entity that hostname
 ...
 *    handleDirectorySite below. `/map` (map entity only) falls through to
 *    the SPA (a client-side route resolves the map by hostname itself; see
 *    CustomDomainMap.jsx). Domains that don't resolve, or aren't active yet,
 *    get an honest static response rather than silently falling into the
 *    branded domain's own route table (which would show the wrong content).
 *
...
async function handleCustomDomain(host, segments, blobBase) {
  ...
  if (segments.length === 0) {
    ...
    // /directories/:clientSlug/:directorySlug/robots.txt path (also served,
    // for manual inspection/forward-compat, but not a domain root a crawler
    // would ever request).
    pathname = `${basePath}/robots.txt`;
    contentType = "text/plain; charset=utf-8";
