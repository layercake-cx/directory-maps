...
/**
 * DIR-E4: does this map read its pins live from a directory instead of its
 * own listings? Checked first, ahead of the CDN snapshot attempt below —
 * snapshots are generated from `listings` at map-publish time and know
 * nothing about directory-sourced entries, so a directory-linked map always
 * takes the live-query path (see docs/DIRECTORIES.md §4.7: "resolved at
 * read time, not sync time").
 */
async function resolveDirectoryAssociation(supabaseClient, mapId) {
  const { data } = await supabaseClient
    .from("directory_map_associations")
    .select("directory_id")
    .eq("map_id", mapId)
    .maybeSingle();
  return data?.directory_id ?? null;
}

/**
 * Check whether a fetched snapshot still matches the map's current
 * publication. The snapshot is generated by a fire-and-forget Edge Function
 * call after publish (see AdminMapDashboard/ClientMapDashboard) — if that
 * call fails or times out, the CDN blob keeps serving a previous publish
 * (e.g. one that still had a removed logo) even though the DB-side
 * publication is correct.
 *
 * Fails open (returns true) on any error so a DB outage doesn't take down
 * the snapshot fast-path this check exists alongside.
 */
async function isSnapshotFresh(supabaseClient, mapId, snapshot) {
  try {
    const { data } = await supabaseClient
      .from("maps")
      ...
      .eq("id", mapId)
      .maybeSingle();
    if (!data?.current_publication_id) return true;
    return data.current_publication_id === snapshot.publicationId;
  } catch {
    return true;
  }
}
...
export default function EmbedMap({ mapId: mapIdProp, overlay = null } = {}) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // ── DIR-E4: directory-sourced map? ──────────────────────────────────
        // Checked ahead of the CDN snapshot, which only ever contains this
        // map's own `listings` as of its last publish.
        const directoryId = await resolveDirectoryAssociation(supabase, mapId);
        ...
        if (directoryId && !cancelled) {
          ...
          const [
            ...
            supabase
              ...
              .eq("id", mapId)
              .single(),
            supabase.from("public_directory_entries").select("*").eq("directory_id", directoryId),
            supabase
          ...
          // Normalize directory shapes into the listing/group shape the rest
          // of this component (and PublishedMapView) already expects, so no
          // downstream rendering code needs to know directories exist.
          const normalizedListings = (entries ?? []).map((e) => ({
            ...e,
            group_id: e.directory_group_id ?? null,
          }));
          const normalizedGroups = (dGroups ?? []).map((gr) => ({ ...gr, theme_json: null }));
          ...
          if (!resolvedPublication && m?.published_config) {
            resolvedPublication = normalizePublicationConfig(m.published_config);
          }
        ...
        const snapshotUsable =
          snapshot && (await isSnapshotFresh(supabase, mapId, snapshot));

        if (snapshotUsable && !cancelled) {
  ...
  const groupsForEmbed = useMemo(() => {
    ...
    // so byId/byName lookups in mergeGroupWithPublication could spuriously
    // match a same-named group left over from this map's own publish
    // history. Keep the directory's own `color`, no per-group marker
    // overrides (directory_groups has no theme_json column).
    if (isDirectorySourced) {
      return (groups || []).map((gr) => ({ ...gr, theme_json: null }));
    }
  ...
  }, [groups, publicationConfig, isDirectorySourced]);
