...
import { listAttachedCategorisations, categoryColumnName, resolveImportedListingTerms, applyImportedListingTerms } from "../../lib/categorisations.js";
...
export default function AdminMapData() {
  ...
  useEffect(() => {
    let alive = true;
    if (!mapId) return;
    listAttachedCategorisations("map", mapId)
      .then((cats) => { if (alive) setCategorisations(cats); })
      .catch(() => {});
    return () => { alive = false; };
  }, [mapId]);
  ...
  useEffect(() => {
    (async () => {
      try {
        const [{ data: c }, { data: m }, { data: g }, { data: ds }, l, { count }, assoc] = await Promise.all([
      ...
      await Promise.all(
        (listings || []).map(async (listing) => {
          ...
          try {
            ...
            if (method === "manual" && /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(String(listing.id || ""))) {
              method = "csv";
            }
          } catch { method = "manual"; }
  ...
  async function connectGoogle() {
    try {
      ...
      if (error) {
        ...
        throw new Error(isGenericFailure ? `${msg}\n\nCheck: Edge Functions are deployed (google_oauth_start etc.) and VITE_SUPABASE_URL points to the correct project.` : msg);
      }
  ...
  async function onPickFile(file) {
    ...
    setRows(objs); setPreview(objs.slice(0, 20));
  ...
  function downloadTemplate() {
    ...
    // Append one column per categorisation attached to this map, same pipe-delimited convention.
  ...
  async function doImport(mode) {
    ...
    rows.forEach((r, idx) => {
      ...
      cleaned.push({
        ...
        group_id: groupKey ? lookup.get(groupKey) ?? null : null,
        is_active: boolish(r.is_active) ?? true,
        source: "csv",
      });
    });

    if (errors.length) { setErr(errors.slice(0, 40).join("\n") + (errors.length > 40 ? `\n… +${errors.length - 40} more` : "")); return; }
    ...
    try {
      ...
      const toUpsert = cleaned.map((row) => { const out = {}; for (const key of UPSERT_KEYS) { if (Object.prototype.hasOwnProperty.call(row, key)) out[key] = row[key]; } return out; });
      ...
      // Resolve category_<key> columns against categorisations attached to this map.
  ...
  async function handleAttachDirectory() {
    ...
    try {
      ...
      recordAdminEvent(supabase, {
        ...
        clientId,
        mapId,
        meta: { client_id: clientId, map_id: mapId, directory_id: selectedDirectoryId, source: "admin_map" },
      });
    } catch (e) {
      setDirectoriesError(e?.message ?? String(e));
    } finally {
  ...
  async function handleDetachDirectory() {
    ...
    try {
      ...
      recordAdminEvent(supabase, {
        ...
        clientId,
        mapId,
        meta: { client_id: clientId, map_id: mapId, directory_id: detachedDirectoryId, source: "admin_map" },
      });
    } catch (e) {
      setDirectoriesError(e?.message ?? String(e));
    } finally {
        ...
        {activeTab === "drive" && (
          <div style={{ display: "grid", gap: 16 }}>
            <p style={{ margin: 0, fontSize: 13, opacity: 0.7, maxWidth: 560 }}>
              Connect a cloud spreadsheet and sync this map automatically. Syncing replaces all existing listings.
            </p>

            {listings.length > 0 && !integrationLinked && (
              <Alert color="yellow" variant="light" title="Existing data will be replaced">
                This map has {listings.length} listing{listings.length !== 1 ? "s" : ""}. Connecting an integration and syncing will overwrite them.
              </Alert>
            )}
          ...
          <div className="admin-card" style={{ padding: 20, display: "grid", gap: 16 }}>
            ...
            <div>
              ...
              <Text size="xs" c="dimmed" mb={12}>
                Must have a <code>name</code> column. Download the template for the full column list.
                Importing replaces all existing data on this map.
              </Text>

              {(listings.length > 0 || integrationLinked) && (
                <Alert color="yellow" variant="light" mb="md" title="This will overwrite existing data">
                  {integrationLinked
                    ? "An active integration is connected. Uploading a CSV will disconnect it and replace all synced listings."
              ...
              <input type="file" accept=".csv" onChange={(e) => onPickFile(e.target.files?.[0])} style={{ fontSize: 13 }} />
            ...
            <label style={{ display: "flex", gap: 10, alignItems: "center", fontSize: 13, cursor: "pointer" }}>
              ...
              Automatically geocode rows missing lat/lng coordinates
            </label>
            ...
            <Group gap="xs" wrap="wrap">
              <Button size="sm" onClick={() => rows.length && setImportChoiceOverlayOpen(true)} disabled={importing || rows.length === 0} loading={importing}>
                {importing ? "Importing…" : `Import ${rows.length} row${rows.length !== 1 ? "s" : ""}`}
              </Button>
              <Button size="sm" variant="default" component={Link} to={`/admin/clients/${encodeURIComponent(clientId)}/maps/${encodeURIComponent(mapId)}`}>
                Done
              </Button>
            </Group>
              ...
              <div>
                ...
                <div style={{ overflowX: "auto", border: "1px solid var(--lc-border)", borderRadius: 8 }}>
                  <table className="admin-table" style={{ marginTop: 0, fontSize: 12 }}>
                    <thead>
                      <tr>{["name","postcode","country","lat","lng","group_name"].map((h) => <th key={h}>{h}</th>)}</tr>
                    </thead>
                    <tbody>
                      {preview.map((r, i) => (
                        <tr key={i}>
                          <td>{r.name}</td><td>{r.postcode}</td><td>{r.country}</td>
                        ...
                        <tr key={listing.id} style={{ borderBottom: "1px solid var(--lc-border)" }}>
                          ...
                          <td style={{ padding: "8px 12px" }}>
                            <Group gap={4} justify="flex-end">
                              <Button size="xs" variant="subtle" leftSection={<Pencil size={12} />} onClick={() => openEditManual(listing)}>Edit</Button>
                              <Button size="xs" variant="subtle" color="red" leftSection={<Trash2 size={12} />} onClick={() => deleteManualEntry(listing)} loading={deletingId === listing.id} disabled={!!deletingId}>Delete</Button>
                            </Group>
                          </td>
                        </tr>
                      );
          ...
          <div style={{ display: "grid", gap: 16 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, justifyContent: "space-between", flexWrap: "wrap" }}>
              <div>
                ...
                <Text size="xs" c="dimmed">Edit logo backgrounds. Source shows where each listing came from.</Text>
              </div>
            ...
            <div style={{ overflowX: "auto", border: "1px solid var(--lc-border)", borderRadius: 8 }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, tableLayout: "fixed", minWidth: 860 }}>
                <colgroup>
                  <col style={{ width: "20%" }} /><col style={{ width: "18%" }} /><col style={{ width: "10%" }} />
                  <col style={{ width: "2px" }} /><col style={{ width: "8%" }} /><col style={{ width: "2px" }} />
                  <col style={{ width: "10%" }} /><col style={{ width: "32%" }} />
                </colgroup>
                <thead>
                  <tr style={{ textAlign: "left", borderBottom: "1px solid var(--lc-border)", background: "rgba(0,0,0,0.02)" }}>
                ...
                <tbody>
                  {pageListings.map((listing) => {
                    ...
                    return (
                      <tr key={listing.id} style={{ borderBottom: "1px solid var(--lc-border)" }}>
                        <td style={{ padding: "8px 10px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }} title={listing.name || "—"}>{listing.name || "—"}</td>
                        <td style={{ padding: "8px 10px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }} title={groupNameById.get(listing.group_id) || "—"}>{groupNameById.get(listing.group_id) || "—"}</td>
                        ...
                        <td style={{ padding: "8px 10px" }}>
                          <div style={{ display: "inline-flex", alignItems: "center", gap: 6, flexWrap: "nowrap" }}>
                            {currentBg ? <span title={currentBg} style={{ width: 12, height: 12, borderRadius: 2, background: currentBg, border: "1px solid rgba(0,0,0,0.2)", flexShrink: 0 }} /> : <span style={{ opacity: 0.5, fontSize: 11 }}>none</span>}
                            {LOGO_BG_SWATCHES.map((swatch) => {
                              const selected = currentBg === swatch.value || (!currentBg && swatch.value === "");
                              return (
                                <button key={swatch.label} type="button" onClick={() => updateListingLogoBg(listing.id, swatch.value)} title={swatch.label}
          ...
          <div style={{ position: "fixed", inset: 0, zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.45)" }} onClick={() => setImportChoiceOverlayOpen(false)}>
            <div className="admin-card" style={{ padding: 24, maxWidth: 380, width: "100%", margin: 16, boxShadow: "0 8px 32px rgba(0,0,0,0.2)" }} onClick={(e) => e.stopPropagation()}>
              ...
              <Text size="xs" c="dimmed" mb={16}>
                You have {listings.length} existing listing{listings.length !== 1 ? "s" : ""}. Replace them or add to them.
              </Text>
              <Stack gap="xs">
                <Button fullWidth color="red" variant="light" type="button" onClick={() => doImport("overwrite")} disabled={importing} loading={importing}>Replace all existing data</Button>
                <Button fullWidth variant="default" type="button" onClick={() => doImport("append")} disabled={importing}>Add to existing data</Button>
                <Button fullWidth variant="subtle" color="gray" type="button" onClick={() => setImportChoiceOverlayOpen(false)}>Cancel</Button>
              </Stack>
            </div>
          </div>
        )}
              ...
              <form onSubmit={saveManualEntry}>
                <Stack gap="sm">
                  ...
                  <Group gap="xs" justify="flex-end" mt={4}>
                    ...
                    <Button size="sm" type="submit" loading={savingManual}>{manualModal === "new" ? "Add entry" : "Save changes"}</Button>
                  </Group>
                </Stack>
              </form>
            </div>
